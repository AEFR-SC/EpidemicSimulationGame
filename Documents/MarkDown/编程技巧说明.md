...
之前是`CharcterObject`的部分代码，源码下面还有`Doctors` `Peoples`两个类都是继承`CharcterObject`的。



### 使用私有属性

因为有时候类里的函数只需要自身调用，无序外部调用，因此可以在名称前加上一条下划线`_`来表示这是私有属性。
...
Git可以很好的管理你的代码， 可以恢复任何一个commit的版本， 还有不同的分支。
